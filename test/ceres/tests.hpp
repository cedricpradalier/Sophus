...
#include <sophus/spline_error.hpp>
...
struct LieGroupCeresTests {
  ...
  bool testAll() {
    ...
    std::cerr << "Spline test: ";
    passed &= testSpline() != nullptr;
    processTestResult(passed);

    std::cerr << "SplineError test: ";
    passed &= testSplineError() != nullptr;
    processTestResult(passed);
    return passed;
  }
        ...
        switch (ku.segment_case) {
            case SegmentCase::first:
                cost = new ceres::AutoDiffCostFunction<TestSplineFunctor, LieGroupd::DoF,
                     ...
                     LieGroupd::num_parameters,
                     LieGroupd::num_parameters>(
                             new TestSplineFunctor(ku.segment_case,ku.u,group_vec[i]));
  ...
  std::shared_ptr<BasisSpline<LieGroupd>> testSplineError(int n_knots=-1) {
      using SES = SplineErrorSupport<double,LieGroup_>;
      using Splined = typename SES::Splined;
      using Functor = TestLieGroupCostFunctor;
    if (group_vec.empty()) 
        return std::shared_ptr<Splined>();
    if (n_knots<0) {
        n_knots = 3 * group_vec.size() / 4;
    }
    // Running Lie group spline approximation
    std::vector<LieGroupd> control_poses(n_knots,LieGroupd());
    std::shared_ptr<Splined> spline(new Splined(control_poses, -1.0, float(group_vec.size()+2)/(n_knots-1)));
    ceres::Problem problem;
